...
def test_registry_remove_method(registry_infra):
    ...
    with pytest.raises(registry_infra.error, match=f'Requested to remove item with key {key_to_remove}, which does not exist.'):
        registry_infra.object.remove(key_to_remove)
...
def test_registry_pop_method(registry_infra):
    ...
    with pytest.raises(registry_infra.error, match=f'Requested to pop item with key {key_to_pop}, which does not exist.'):
...
def test_registry_get_method(registry_infra):
    ...
    with pytest.raises(registry_infra.error, match=f'Requested to get item with key {non_existing_key}, which does not exist.'):
